...
function aimachine(ailevel){
	...
	switch(aistan){
        ...
        for(var j in wysortowane){
                        ...
                        for(var j = 0;j < curr_hexes.length;j++){
                            if(i != j && curr_hexes[i].x+'#'+curr_hexes[i].y != closcode && curr_hexes[j].x+'#'+curr_hexes[j].y != closcode/* && curr_hexes[j].color != -1*/){
                                ...
                                if(closcode in curr_hexes[i].dmap && closcode in curr_hexes[j].dmap && curr_hexes[j].x+'#'+curr_hexes[j].y in curr_hexes[i].dmap/* && 
                                    ...
                                    var prst = d1 > 2 && d1 > (d2 + d3)*0.7 && !(d2 > (d1 + d3)*0.7) && d1 > d2
                                    if(prst){
                                        ok2_ = false
            ...
            possible_targets.sort((a,b)=>((a.dist+Math.pow(0.9,1+a.value))-(b.dist+Math.pow(0.9,1+b.value))))
        ...
        case 1.3:
            if(possible_targets_ix >= possible_targets.length){
                ...
                for(var ct = 0;ct<6;ct++){
                    ...
                    var embarkingTargets = getEmbarkingTargets(newDistmap,kolej)
                    ...
                    if(newDistmap.distmaps[code].alliegance[MAX_TURNS-1] != kolej){
                        ...
                        var result = tryPutUnderAttack(newDistmap,tested_target.hex.x,tested_target.hex.y,kolej,ulepszyns % 2 == 1,embarkingTargets,behind_score,failedUses)
                        //if(!result){
                    ...
                    *//*
                    ...
                    var score__1 = 0
                    for(var i = 0;i<MAX_TURNS;i++){ score1 += newDistmap.score[kolej][i] * Math.pow(1/2.1,i) }
                    var score__2 = 0
                    for(var i = 0;i<MAX_TURNS;i++){ score2 += dbetter.score[kolej][i] * Math.pow(1/2.1,i) }
                ...
                for(var j in pole_z_tratwami.hex.units){
                    ...
                    for(var k in map){
                        ...
                        if(code in miejsca_do_wysłania_tratw){
                            ...
                            var action = {type:'move',by:'speculation2',rucho:rucho,ruchk:ruchk,il:liczba,from:[pole_z_tratwami.x,pole_z_tratwami.y],destination:leadPath(pole_z_tratwami.hex.x,pole_z_tratwami.hex.y,ruchk,rucho),leadedPath:getLeadedPath(pole_z_tratwami.hex.x,pole_z_tratwami.hex.y,ruchk,rucho)}
            ...
            if(unit.d == kolej && unit.actions.length > 0 && unit.actions[0].type == 'move'){
                ...
                //if(time <= 2){
                    ...
                    for(var ld in realSfKeys){
                        ...
                        if(Math.max(0,(disttotown-Math.max(0,zas[unit.rodz])))/szy[unit.rodz] <= 2){
    ...
    constructor(unit,hex){
        ...
        if(unit.actions != undefined){
            ...
            if(this.ruchy > 0){
                ...
                this.actions.push({type:'move',by:'real',rucho:rucho,ruchk:ruchk,il:unit.il,from:[hex.x,hex.y],destination:leadPath(hex.x,hex.y,ruchk,rucho),embarking:embarkingPointFromLeadedPath(hex.x,hex.y,ruchk,rucho),distant:false,leadedPath:getLeadedPath(hex.x,hex.y,ruchk,rucho)})
            }
            if(this.celu != -1 && this.celd in unix){
                var aimedunit = unix[this.celd][this.celu]
                this.actions.push({type:'aim',by:'real',celu:this.celu,celd:this.celd,hex_x:aimedunit.x,hex_y:aimedunit.y,kolor:this.kolor,distant:false})
            }
            if(this.rozb > 0){
                this.actions.push({type:'build',by:'real',size:this.rozb,distant:false})
            }
...
function copyDistmaps(dm){
    ...
    for(var i in hexesWithUnits){
        ...
        newDistmaps[code] = {maps:{}, hex:bhex, potentialEmbarkings:dm.distmaps[code].potentialEmbarkings, potentialtocome:[], realtocome:[], defence:[],alliegance:allTurns(),fromenemy:allMoves(),fromally:allMoves(),frontline:false}
...
function distmapsFromUnit(){
    ...
    for(var i in hexesWithUnits){
        ...
        distmaps[code] = {maps:{}, hex:bhex, potentialEmbarkings:[], potentialtocome:[], realtocome:[], defence:[],alliegance:allTurns(),fromenemy:allMoves(),fromally:allMoves(),frontline:false}
                        ...
                        */
                        ...
                        if(!pathIsThroughCrowdedCity(dm,distmap.hex.x,distmap.hex.y,ruchk,rucho)){
                            ...
                            if(frontline == undefined || true){
                                unit.legalActions.push([{type:'move',by:'speculation2',rucho:rucho,ruchk:ruchk,il:unit.il,from:[distmap.hex.x,distmap.hex.y],destination:lnp,embarking:embarking,distant:false,leadedPath:getLeadedPath(distmap.hex.x,distmap.hex.y,ruchk,rucho)}])
                                if(unit.il > 10 && distmap.hex.units.length <= 3)
                                    unit.legalActions.push([{type:'move',by:'speculation2',rucho:rucho,ruchk:ruchk,il:unit.il-10,from:[distmap.hex.x,distmap.hex.y],destination:lnp,embarking:embarking,distant:false,leadedPath:getLeadedPath(distmap.hex.x,distmap.hex.y,ruchk,rucho)}])
                                        ...
                                        if(aimedunit != null && aimedunit.d != unit.d && miaruj(unit,aimedunit,hex.hex)){
                                            ...
                                            if(ds != undefined && ds <= zas[unit.rodz] && turnPrediction < 2){  
                                                unit.legalActions.push([
                                                    {type:'move',by:'speculation2',rucho:rucho2,ruchk:ruchk2,il:unit.il,from:[distmap.hex.x,distmap.hex.y],destination:[hex.hex.x,hex.hex.y],embarking:embarking2,distant:false,leadedPath:getLeadedPath(distmap.hex.x,distmap.hex.y,ruchk2,rucho2)},  //not a real situation, but it solves some problem
                                                ...
                                                if(unit.il > 10 && distmap.hex.units.length <= 3)
                                                    unit.legalActions.push([
                                                        {type:'move',by:'speculation2',rucho:rucho2,ruchk:ruchk2,il:unit.il-10,from:[distmap.hex.x,distmap.hex.y],destination:[hex.hex.x,hex.hex.y],embarking:embarking2,distant:false,leadedPath:getLeadedPath(distmap.hex.x,distmap.hex.y,ruchk2,rucho2)},
                        ...
                        if(hex.hex.units.length > 0){
                            ...
                            if(hex.hex.units.length > 0){
                                ...
                                if(aimedunit != null && aimedunit.d != unit.d && miaruj(unit,aimedunit,hex.hex) && ds != undefined && ds <= zas[unit.rodz]){
                                    unit.legalActions.push([
                                        {type:'aim',by:'speculation2',celu:aimedunit.id,celd:aimedunit.d,il:unit.il,from:[distmap.hex.x,distmap.hex.y],hex_x:hex.hex.x,hex_y:hex.hex.y,destination:[hex.hex.x,hex.hex.y]},
                                    ])
                ...
                //if(hex.hex.z > 0 || hex.hex.units.length > 0){
                    ...
                    */
                    ...
                    if(hexkey in possibleEmbarkings[unit.d]){
                        ...
                        //if(il < 10)
                        //    continue
                        var move = [{type:'move',by:'speculation2',rucho:rucho,ruchk:ruchk,il:unit.il,from:[distmap.hex.x,distmap.hex.y],destination:leadPath(distmap.hex.x,distmap.hex.y,ruchk,rucho),embarking:null,leadedPath:getLeadedPath(distmap.hex.x,distmap.hex.y,ruchk,rucho)}]
...
function tryPutUnderAttack(dm, x, y, color, thinkmore, embarkingTargets, behind_score,failedUses){
    var distmaps = dm.distmaps
...
//     }
    ...
    var interestingUnits = destinationmap[code] ? destinationmap[code] : []
    ...
    interestingUnits = interestingUnits.filter(x=>
        ...
        (/*x.unit.actions[0].by != 'speculation2' && */(x.unit.actions[0].type == 'move' && x.action[0].type == 'move' && (x.action[0].rucho.length <= szy[x.unit.rodz]+2 || x.unit.actions[0].rucho.length+1+zas[x.unit.rodz] > x.action[0].rucho.length)/* && x.unit.actions[0].type != 'move' && x.unit.actions[0].type != 'aim'*/))))
    ...
    for(var i in farFromFront){
        ...
        if(fff.code in distmaps){
            ...
            if(distmap.hex.heks.z > 0){
                if(unit.actions.length > 0 && unit.actions[0].type == 'move' && distmap.hex.units[0] == unit && unit.actions[0].il > unit.il-10 && fff.unitIx == 0){
                    continue
                }
                var moving = 0
                for(var j in distmap.hex.units){
            ...
            if(unit.actions.length > 0 && unit.actions[0].type == 'move'){
                ...
                if(!locked){
                    ...
                    for(var key in realSfKeysOriginal){
                        ...
                        var disttime2 = (distance(realSfKeysOriginal[key].hex.x,realSfKeysOriginal[key].hex.y,udistx2,udisty2)-zas[unit.rodz])/szy[unit.rodz]

                        if(disttime <= 2 || disttime2 > disttime && dist > 0/* && unit.actions.length > 0 && unit.actions[0].type == 'move' && unit.actions[0].rucho.length > 0*/){
                            ok = false
                            break
                        }
                    }
                ...
                if(key1 in possibleDestinations){
                    ...
                    for(var key2 in realSfKeysOriginal){
                        ...
                        if(key1 != key2 && key2 in dfrou.distmaps/* && dfrou.distmaps[key2].alliegance[MAX_TURNS-1] != -1/* && dfrou.distmaps[fff.code].alliegance[MAX_TURNS-1] != kolej*/){
                            ...
                            if(d1 != null && d2 != null && d3 != null && d1 > d2 && d1 > szy[unit.rodz] && d1 > (d2+d3) * 0.7 && !(d2 > (d1+d3) * 0.7)){
                                delete possible[key1]
                                break
                            }
                        }
            ...
            for(var j in unit.legalActions){
                ...
                if(lac[0].type == 'move' && lac.length == 1 && lac[0].il >= unit.il-10){
                    ...
                    if(warnung(distmap,unit,lac,fff.unitIx,false)){
                        continue
                    }
