                ...
                for(var j = 0;j<scian;j++){
                    ...
                    if(lmap.color == k){
                        ...
                        for(var k in lmap.dmap){
                            ...
                            if(dmapelem.color == kolej && dmapelem.x+'#'+dmapelem.y in dfrou.distmaps && dmapelem.dist >= 0){
...
function prepareLargeMap(dm,t){
    ...
    for(var i = 0;i<scian;i++){
        ...
        for(var j = 0;j<scian;j++){
            ...
            large_map[i][j] = {x:i,y:j,color:ckolor,dmap:{},closestelem:null,dist:Infinity}
        }
    }
    ...
    for(var key in dm.distmaps){
        ...
        if(okoks){
            ...
            if(druu == color && (mod == undefined || !(key in mod) || mod[key] == color) || druu != color && (mod != undefined && key in mod && mod[key] == color)){
                ...
                behind[key] = {hex:large_map[coords[0]][coords[1]],value:dm.distmaps[key].hex.heks.z}
    ...
    constructor(unit,hex){
        ...
        if(unit.actions != undefined){
            ...
            if(this.ruchy > 0){
                ...
                this.actions.push({type:'move',by:'real',rucho:rucho,ruchk:ruchk,il:unit.il,from:[hex.x,hex.y],destination:leadPath(hex.x,hex.y,ruchk,rucho),embarking:embarkingPointFromLeadedPath(hex.x,hex.y,ruchk,rucho)})
            }
                    ...
                    if(unit.actions.length > 0){
                        ...
                        for(var j in unit.actions){
                            ...
                            if(action.type == 'aim'){             
                                ...
                                if(action.celd == -2)
                                    continue
                                var code = action.hex_x+'#'+action.hex_y//unix[action.celd][action.celu].x + '#' + unix[action.celd][action.celu].y//action.hex_x + '#'+ action.hex_y
                                for(var t = movingDelay;t<MAX_TURNS;t++){
                                    if(zas[unit.rodz] > 1){
                                        distmaps[code].realtocome[t][unit.d] -= -Number(unitAttackStrength) * (t - movingDelay + 1)//Math.min(1.5,t-movingDelay+1)
                                    } else {
                                        distmaps[code].realtocome[t][unit.d] -= -Number(unitAttackStrength)//Math.min(1.5,t-movingDelay+1)
                    ...
                    if(hex.hex.z > 0 || hex.hex.units.length > 0){
                        ...
                        */
                        ...
                        if(!pathIsThroughCrowdedCity(dm,distmap.hex.x,distmap.hex.y,ruchk,rucho)){
                            ...
                            unit.legalActions.push([{type:'move',by:'speculation2',rucho:rucho,ruchk:ruchk,il:unit.il,from:[distmap.hex.x,distmap.hex.y],destination:lnp,embarking:embarking}])
                            if(unit.il > 10 && distmap.hex.units.length <= 3)
                                unit.legalActions.push([{type:'move',by:'speculation2',rucho:rucho,ruchk:ruchk,il:unit.il-10,from:[distmap.hex.x,distmap.hex.y],destination:lnp,embarking:embarking}])
                                    ...
                                    if(aimedunit != null && aimedunit.d != unit.d && miaruj(unit,aimedunit,hex.hex)){
                                        ...
                                        if(ds != undefined && ds <= zas[unit.rodz]){
                                            unit.legalActions.push([
                                                {type:'move',by:'speculation2',rucho:rucho2,ruchk:ruchk2,il:unit.il,from:[distmap.hex.x,distmap.hex.y],destination:[hex.hex.x,hex.hex.y],embarking:embarking2},  //not a real situation, but it solves some problem
...
/*          for(var i in unit.actions){
            ...
            for(var i in lac){
                ...
                if(legalAction.length >= 1 && (legalAction[0].type == 'move' || legalAction[0].type == 'aim') && (legalAction[0].destination != undefined || legalAction[0].hex_x != undefined)){
                    ...
                    if(bestAction[dest_code] == null || 
                        evalUnitAttack(unit,legalAction,dm.model) > evalUnitAttack(unit,bestAction[dest_code],dm.model) || (evalUnitAttack(unit,legalAction,dm.model)*2 > evalUnitAttack(unit,bestAction[dest_code],dm.model) &&
                        legalAction.length == 1 && (bestAction[dest_code].length == 1 || zas[unit.rodz] > 1) && legalAction[0].type == 'aim' && bestAction[dest_code][0].type == 'move' || 
                        legalAction.length > 1 && bestAction[dest_code].length == 1 && legalAction[1].type == 'aim' && bestAction[dest_code][0].type == 'move')){
            ...
            for(var dest_code in bestAction){
                if(bestAction[dest_code] != null && bestAction[dest_code].length >= 1){
                    ...
                    destinationmap[dest_code].push({unit:unit, action:bestAction[dest_code], hex:distmap.hex, potentialEmbarkings: distmap.potentialEmbarkings})
                }
            }
        }
    }
...
function tryPutUnderAttack(dm, x, y, color, thinkmore, embarkingTargets){
    var distmaps = dm.distmaps
...
//     }
    ...
    interestingUnits = interestingUnits.filter(x=>x.unit.actions.length == 0 || (x.unit.actions[0].by != 'speculation2'/* && x.unit.actions[0].type != 'move' && x.unit.actions[0].type != 'aim'*/))
